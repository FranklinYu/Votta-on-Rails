...
Set up the environment variables +DATABASE_URL+, +RACK_ENV+, +RAILS_ENV+,
and +SECRET_KEY_BASE+. Typically +RACK_ENV+ and +RAILS_ENV+ should both be
+production+; +DATABASE_URL+ should be a
...
Note that the +-C+ flag is aliased to +--config+.
